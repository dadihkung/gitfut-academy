          ...
          >
            ...
            <div className="p-6 space-y-6">
              <div className="flex items-center justify-between bg-black/30 p-4 rounded-lg border border-white/5">
                <span className="text-sm font-semibold text-slate-300">Match Status:</span>
              ...
              <div className="bg-black/30 border border-white/5 rounded-lg p-4 space-y-4">
                <h4 className="flex items-center gap-2 text-sm font-bold text-[#FFD700] uppercase tracking-wide">
                  <BookOpen size={16} /> How to Learn This
                </h4>
                ...
                {task.guide.resources.length > 0 && (
                  <div className="pt-3 border-t border-white/5 space-y-2">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Resources</p>
              ...
              {task.status === "Bench" || task.status === "In Play" ? (
                <div className="space-y-4">
                  <label className="block text-sm font-semibold text-slate-300">Demo / Repo URL</label>
                  <input
                    type="url"
                    placeholder="https://github.com/... or Replit link"
                    ...
                    value={demoInput}
                    onChange={(e) => onDemoInputChange(e.target.value)}
                  />
                  <button
                    onClick={onSubmit}
                    disabled={!demoInput}
                  ...
                  >
                    <Play size={18} fill="currentColor" /> Submit for VAR Review
                  </button>
                </div>
              ) : (
                <div className="bg-[#00E676]/10 border border-[#00E676]/30 p-4 rounded-lg text-center space-y-2">
                  {task.status === "Goal Scored!" ? (
                    <>
                      ...
                      <p className="font-bold text-[#00E676]">Goal Approved by Coach Usamah!</p>
                      <p className="text-sm text-slate-400">Stats upgraded applied to Ibrahim's FUT Card.</p>
                    </>
                  ) : (
                    <>
                      <Clock className="mx-auto text-yellow-500 w-12 h-12" />
                      <p className="font-bold text-yellow-500">Under VAR Review</p>
                      <p className="text-sm text-slate-400">Awaiting Coach Usamah's approval.</p>
